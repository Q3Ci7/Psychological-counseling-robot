...
// 启动任务
void start_check_playback_task()
{
    // 如果任务句柄不为空，说明任务已经创建了，需要先删除现有任务
    if (check_task_handle != NULL) {
        vTaskDelete(check_task_handle);
        check_task_handle = NULL; // 任务删除后，将句柄设为NULL
    }

    // 创建新任务
    if (xTaskCreate(checkplystation, "checkplystation", 2048, NULL, 5, &check_task_handle) != pdPASS) {
        // 处理任务创建失败的情况
